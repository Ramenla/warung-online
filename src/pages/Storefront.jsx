...
export default function Storefront() {
  ...
  // Group products by kategori for dynamic sections
  const groupedByCategory = {};
  products.forEach(p => {
    if (!groupedByCategory[p.kategori]) groupedByCategory[p.kategori] = [];
    groupedByCategory[p.kategori].push(p);
  });
  ...
  return (
    <div className="min-h-screen bg-neo-bg relative pb-32">
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-8">
        ...
        {!isLoading && (
            <>
                {/* Dynamic Category Sections (when NOT searching) */}
                {!isSearching && categoryKeys.map(cat => (
                  <section key={cat}>
                    <h2 className="text-xl md:text-2xl font-black mb-1 inline-block bg-black text-white px-3 py-1 transform -rotate-1 border-2 border-neo-orange uppercase">
                      {cat}
                    </h2>
