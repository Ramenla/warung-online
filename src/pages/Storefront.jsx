...
export default function Storefront() {
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const searchMatch = product.nama
      .toLowerCase()
  ...
  return (
    <div className="min-h-screen bg-white relative pb-32">
            ...
            {!isSearching &&
              ...
              sections.map((section) => {
                const sectionProducts = products.filter((p) =>
                  (section.produk_ids || []).includes(p.id),
                );
                  ...
                  <section key={section.id}>
                    ...
                    <div className="flex overflow-x-auto gap-3 md:gap-4 pb-4 pt-2 px-1 snap-x scrollbar-hide">
                      {sectionProducts.map((product) => (
                        <div
                          key={product.id}
                          className="flex-shrink-0 w-36 md:w-48 snap-start"
                        >
                          <ProductCard
                            product={product}
      ...
      <div
        className={`fixed bottom-0 left-0 w-full bg-white border-t-4 border-black z-50 transform transition-transform duration-300 rounded-t-xl overflow-hidden ${isDrawerOpen ? "translate-y-0" : "translate-y-full"}`}
      >
        {selectedMobileProduct && (
          <div className="p-4">
              ...
              <div className="flex gap-3">
                <div className="w-20 h-20 bg-neo-teal border-2 border-black rounded shrink-0 overflow-hidden">
                  {selectedMobileProduct.image_url ? (
                    <img
                      src={selectedMobileProduct.image_url}
                      alt={selectedMobileProduct.nama}
                      className="w-full h-full object-cover"
                    />
                  ) : (
                    <span className="font-bold text-xs flex h-full items-center justify-center text-black">
                      FOTO
                    </span>
                  )}
                </div>
                <div>
                  <h3 className="font-bold text-sm line-clamp-2 text-black leading-tight mb-1">
                    {selectedMobileProduct.nama}
                  </h3>
                  <p className="font-black text-rose-600 text-lg">
                    {formatRupiah(selectedMobileProduct.harga)}
                  </p>
                  <p className="text-xs font-bold text-gray-500 mt-1">
                    Stok: {selectedMobileProduct.stok || 0}
                  </p>
                </div>
              </div>
              <button
                onClick={() => setIsDrawerOpen(false)}
                className="w-8 h-8 flex items-center justify-center bg-red-500 text-white border-2 border-black font-black hover:bg-red-600 active:translate-y-0.5 shadow-[2px_2px_0_0_black]"
              >
                X
              </button>
            </div>
            ...
            {(selectedMobileProduct.stok || 0) > 0 ? (
              <>
                <div className="flex justify-between items-center py-4 border-y-2 border-dashed border-gray-300 mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="w-10 text-center font-bold text-black border-2 border-black py-1">
                      {mobileQty}
                    </span>
                    <button
                      onClick={() =>
              ...
              <div className="py-4 text-center">
                <p className="font-black text-red-500 bg-red-100 p-2 border-2 border-red-500 rounded">
                  Stok Barang Habis
                </p>
              </div>
            )}
