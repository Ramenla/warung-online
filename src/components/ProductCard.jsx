...
const ProductCard = ({ product }) => {
  // Destructure data sesuai kolom database Supabase
  const { nama, kategori, satuan, harga } = product;
  const { addToCart } = useContext(CartContext);

  // Helper formatting sesuai instruksi (meskipun di Context ada, kita buat lokal/manual di sini)
  ...
  // Normalisasi data produk untuk CartContext agar tidak error
  const handleAddToCart = () => {
    addToCart({
      ...product,
      name: nama,      // Map 'nama' ke 'name'
      price: harga,    // Map 'harga' ke 'price' (PENTING untuk total price)
      unit: satuan,
      category: kategori
    });
  };
  ...
  return (
    <div className="w-full bg-white border-2 md:border-4 border-black rounded-xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] md:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] overflow-hidden transition-transform hover:-translate-y-1 hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] md:hover:shadow-[12px_12px_0px_0px_rgba(0,0,0,1)]">
      {/* Gambar Produk */}
      <div className="h-20 md:h-40 bg-neo-teal border-b-2 md:border-b-4 border-black flex items-center justify-center overflow-hidden">
        {product.image_url ? (
      ...
      <div className="p-2 md:p-5">
        {/* Badge Kategori pakai warna Ungu */}
        ...
        <h2 className="mt-1 md:mt-3 text-sm md:text-xl font-extrabold text-black leading-tight min-h-[2.5rem] md:min-h-[3.5rem] line-clamp-2">
          {nama}
        </h2>
        <p className="mt-1 text-gray-700 font-medium text-xs md:text-base">Satuan: {satuan}</p>

        <div className="mt-2 md:mt-5 flex flex-col md:flex-row md:items-center justify-between gap-2 md:gap-0">
          ...
          {/* Tombol Tambah pakai warna Orange */}
          <button 
            onClick={product.stok > 0 ? handleAddToCart : null}
            disabled={!product.stok || product.stok <= 0}
            className={`font-bold py-1 px-2 md:py-2 md:px-3 text-xs md:text-sm border-2 border-black rounded-lg shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] md:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all w-full md:w-auto text-center
            ${product.stok > 0 
          ...
          >
            {product.stok > 0 ? '+ Beli' : 'Stok Habis'}
          </button>
        </div>
      </div>
    </div>
